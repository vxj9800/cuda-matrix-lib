...
void ode45(std::function<cu_mat (const cu_mat &t, const cu_mat &x, const cu_mat &params)> der, double t0, double tstep, double tf, cu_mat y0, cu_mat der_params)
{
	std::cout << "it reached here." << std::endl;
	...
	static cu_mat t0_cu = t0, tstep_cu = tstep, tf_cu = tf;
	static cu_mat cu_1000 = 1000, cu_16 = 16, cu_2 = 2;// Some needed numbers
	static cu_mat t = stepspace(t0,tf,tstep), rtol = 0.001, atol = 1e-6, hmax = 0, h = tstep_cu/cu_1000;
	static cu_mat k1 = y0, k2 = y0, k3 = y0, k4 = y0, k5 = y0, k6 = y0, k7 = y0, y1 = y0, z1 = y0, err = 0, s = 0;
	static cu_mat t_start = 0, t_end = 0, hmin = 0, sf = 0.8;
	size_t n_fail = 0;
	...
	{
		t_start = t(loop,1); t_end = t(loop+1,1); h = tstep_cu/cu_1000; // There is some problem in this line
		hmax = (t_end-t_start)/10.0; hmin = cu_16*eps(double(t_start)); n_fail = 0;
